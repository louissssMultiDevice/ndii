...
if (isCmd) {
    ...
    console.log(`
╭┈❲ ${chalk.redBright(ndii.user?.name || 'BOT')}
...
if (db.game.family100?.[m.chat] && !isCmd) {
  ...
  if (!isCorrect) {
    ndii.sendMessage(m.chat, { react: { text: "❌", key: m.key } })
    return
  }
...
break
      ...
      if (global.autogempa) {
        ...
        await ndii.sendMessage(m.chat, { text: "✅ Auto notifikasi gempa *aktif* (global, multi channel)." });
      } else {
        await ndii.sendMessage(m.chat, { text: "⛔ Auto notifikasi gempa *dimatikan*." });
      }
      break;
    }
...
case 'get': {
    ...
    try {
        ...
        switch(tipe) {
            case 'img':
                await ndii.sendMessage(m.chat, { image: response.data, caption: "🖼️ Nih gambarnya!" }, { quoted: m });
                break;
            case 'vid':
                await ndii.sendMessage(m.chat, { video: response.data, caption: "🎬 Nih videonya!" }, { quoted: m });
                break;
            case 'html':
                await ndii.sendMessage(m.chat, { document: response.data, fileName: "file.html", mimetype: "text/html", caption: "📄 File HTML siap dikirim!" }, { quoted: m });
                break;
            case 'zip':
            default:
                await ndii.sendMessage(m.chat, { document: response.data, fileName: "file.zip", mimetype: "application/zip", caption: "🗜️ File ZIP berhasil diambil!" }, { quoted: m });
                break;
        }
    } catch (error) {
    ...
    switch (args[0]) {
        ...
        default: {
            let ndiiGanteng = '✨ Gunakan salah satu perintah berikut:\n- backup auto\n- backup all\n- backup database\n- backup session';
            let buttons = [
                {
                    buttonId: "backup",
            ...
            case 'ping':
{
    ...
    const healthStatus = isOverload ? '⚠️ **OVERLOAD**' : '✅ **HEALTHY**';
    const healthEmoji = isOverload ? '🔴' : '🟢';
    ...
    // Pesan utama
    const msgText = `
${healthEmoji} *𝐍𝐀𝐌𝐈-𝐀𝐈 𝐒𝐄𝐑𝐕𝐄𝐑 𝐒𝐓𝐀𝐓𝐔𝐒*
══════════════════════════
...
💾 *MEMORY USAGE*
├ RAM: ${(totalMem - freeMem).toFixed(2)}MB / ${totalMem.toFixed(2)}MB
├ ${createProgressBar(memPercentage)} ${memPercentage}%
├ Heap: ${usedHeap.toFixed(2)}MB / ${heapTotal.toFixed(2)}MB
└ ${createProgressBar(heapPercentage)} ${heapPercentage}%

⚡ *CPU LOAD* (1/5/15 min)
├ ${loadAvg[0].toFixed(2)} / ${loadAvg[1].toFixed(2)} / ${loadAvg[2].toFixed(2)}
└ ${createProgressBar(cpuUsage * 10)} ${cpuUsage}
...
case 'nsfw': {
        ...
        if (!m.isGroup) return m.reply('⚠️ Perintah ini hanya bisa dipakAIdi grup.')
...
Kadang beruntung lho — kamu bisa mendapatkan *limit sampai1000* atau *hadiah akses premium* untuk waktu tertentu! 🎉✨  
